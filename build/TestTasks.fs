...
[<Literal>]
let FableTestPath_input = "tests/FSharpAux.Core.Tests"
[<Literal>]
let FableTestPath_output = "tests/JsNativeTests/fable"
...
module WatchTests =
    ...
    let private fableTestsProcesses =
        [
            "[Fable Core]", dotnet $"fable watch {FableTestPath_input} -o {FableTestPath_output} -s --run npm run test:live" "."
